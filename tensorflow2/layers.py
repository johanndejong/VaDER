...
def encode_monolayer_recurrent(X, D, I, cell_type, n_hidden):
    ...
    if cell_type == "LSTM":
        encoder = tf.compat.v1.nn.rnn_cell.LSTMCell(
            num_units=n_hidden + n_hidden,  # one for mu, one for sigma
            activation=tf.nn.tanh, name="encoder", use_peepholes=True
        )
        (_, (_, hidden)) = tf.compat.v1.nn.static_rnn(encoder, X, dtype=tf.float32)
    else:
        encoder = tf.compat.v1.nn.rnn_cell.GRUCell(
            num_units=n_hidden + n_hidden,  # one for mu, one for sigma
            activation=tf.nn.tanh, name="encoder"
        )
        (_, hidden) = tf.compat.v1.nn.static_rnn(encoder, X, dtype=tf.float32)
...
def decode_monolayer_recurrent(z, D, I, cell_type, n_hidden, output_activation):
    n_hidden = n_hidden[0]
    weight = tf.Variable(tf.random.truncated_normal([n_hidden, I], dtype=tf.float32), name='weight')
    bias = tf.Variable(tf.constant(0.1, shape=[I], dtype=tf.float32), name='bias')

    input = [tf.zeros((tf.shape(input=z)[0], I), dtype=tf.float32) for _ in range(D)]
    if cell_type == "LSTM":
        decoder = tf.compat.v1.nn.rnn_cell.LSTMCell(n_hidden, name="decoder", use_peepholes=True)
        (output, (_, _)) = tf.compat.v1.nn.static_rnn(decoder, input, initial_state=(tf.zeros(tf.shape(input=z)), z), dtype=tf.float32)
    else:
        decoder = tf.compat.v1.nn.rnn_cell.GRUCell(n_hidden, name="decoder")
        (output, _) = tf.compat.v1.nn.static_rnn(decoder, input, initial_state=z, dtype=tf.float32)
    output = tf.transpose(a=tf.stack(output), perm=[1, 0, 2])
    weight = tf.tile(tf.expand_dims(weight, 0), [tf.shape(input=z)[0], 1, 1])
...
def decode_multilayer_recurrent(z, D, I, cell_type, n_hidden, output_activation):
    ...
    for n in n_hidden[1:]:
        hidden = my_dense_layer(hidden, n)
    weight = tf.Variable(tf.random.truncated_normal([n_hidden[-1], I], dtype=tf.float32),
                         name='weight')
    bias = tf.Variable(tf.constant(0.1, shape=[I], dtype=tf.float32), name='bias')

    input = [tf.zeros((tf.shape(input=hidden)[0], I), dtype=tf.float32) for _ in range(D)]
    if cell_type == "LSTM":
        decoder = tf.compat.v1.nn.rnn_cell.LSTMCell(n_hidden[-1], name="decoder", use_peepholes=True)
        (output, (_, _)) = tf.compat.v1.nn.static_rnn(decoder, input, initial_state=(tf.zeros(tf.shape(input=hidden)), hidden),
                                            dtype=tf.float32)
    else:
        decoder = tf.compat.v1.nn.rnn_cell.GRUCell(n_hidden[-1], name="decoder")
        (output, _) = tf.compat.v1.nn.static_rnn(decoder, input, initial_state=hidden, dtype=tf.float32)

    output = tf.transpose(a=tf.stack(output), perm=[1, 0, 2])
    weight = tf.tile(tf.expand_dims(weight, 0), [tf.shape(input=hidden)[0], 1, 1])
    output = tf.matmul(output, weight) + bias
    x_raw = tf.identity(output, name="x_raw")
    x = output_activation(x_raw, name="x_output")
    return x, x_raw
