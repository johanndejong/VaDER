...
def latent_loss(z, mu_c, sigma_c, phi_c, mu_tilde, log_sigma_tilde, K, eps=1e-10):
    if K == 1:  # ordinary VAE
        ...
        def log_pdf(z, mu, sigma):
            def f(i):
                return - tf.square(z - mu[i]) / 2.0 / (eps + sigma[i]) - tf.log(
                    eps + eps + 2.0 * np.pi * sigma[i]) / 2.0

            return tf.transpose(tf.map_fn(f, np.arange(K), dtype=tf.float32), [1, 0, 2])
        ...
        term1 = tf.log(eps + sigma_c)
        term2 = tf.transpose(
            tf.map_fn(lambda i: tf.exp(log_sigma_tilde) / (eps + sigma_c[i]), np.arange(K), tf.float32),
            [1, 0, 2])
        term3 = tf.transpose(
            tf.map_fn(lambda i: tf.square(mu_tilde - mu_c[i]) / (eps + sigma_c[i]), np.arange(K),
                      tf.float32), [1, 0, 2])
        ...
        latent_loss2 = - tf.reduce_sum(gamma_c * (tf.log(eps + phi_c) - log_gamma_c), axis=1)
